 ...
 */
import type { TutorialAnchorId, TutorialChapterId } from './types';
...
/** Your own Leader — what you boost with a Counter, and what you attack WITH. */
const LEADER: TutorialCardRef = { kind: 'leader' };
/** The other player's Leader — what you attack INTO. Keeping these two apart
 *  is not pedantry: they are different cards, and conflating them made every
 *  scripted attack target the attacker's own Leader, which the engine
 *  correctly rejected with "does not belong to the opponent". */
const OPPOSING_LEADER: TutorialCardRef = { kind: 'opposingLeader' };
const own = (cardNumber: string): TutorialCardRef => ({ kind: 'ownCharacter', cardNumber });
const opposing = (cardNumber: string): TutorialCardRef => ({ kind: 'opposingCharacter', cardNumber });
 ...
 */
export const OPENING_HAND = ['OP05-063', 'ST18-004', 'ST18-003', 'ST18-002', 'OP05-068'] as const;
...
export const TUTORIAL_CHAPTERS: ReadonlyArray<{ id: TutorialChapterId; title: string; turn: number }> = [
...
export function beatsForChapter(chapter: TutorialChapterId): TutorialBeat[] {
  return TUTORIAL_SCRIPT.filter((beat) => beat.chapter === chapter);
}

/** Index into TUTORIAL_SCRIPT of the first beat of a chapter, or -1. */
export function firstBeatIndexOfChapter(chapter: TutorialChapterId): number {
  return TUTORIAL_SCRIPT.findIndex((beat) => beat.chapter === chapter);
}
