...
Everything in `public/audio/sfx/` today is a **placeholder**, procedurally
generated by `scripts/audio/build.sh`. Replacing a sound means dropping a new
`.ogg` at the same path. No code change, no registry edit, no rebuild step.
 ...
 pure: log delta -> cues         pure: element -> cue
        ...
        └──────────────┬───────────────┘
          ...
          audio/soundManager.ts  ── factory over injected deps
             throttle · voice caps · priority · detune · ducking
                       ▼
...
1. Pick a cue in `public/audio/sfx/manifest.json`; its `brief` is the
...
- **Bulk regeneration** of placeholders: `bash scripts/audio/build.sh`. This
  overwrites every `.ogg`, so run it when the cue table changes, **not** after
  a real asset has landed.
...
- **All 86 assets are synthetic placeholders.** They are distinguishable and
  correctly paced, but they are not the final sound of the game.
