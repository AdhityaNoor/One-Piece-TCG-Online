...
function writeMarkdown(file: string, selectedSets: readonly string[], rows: readonly UncoveredRow[]): void {
  ...
  const md: string[] = [];
  md.push('# V2 Uncovered Effects', '');
  md.push('_Generated by `npm run effects:v2:uncovered`._', '');
  md.push('| Metric | Count |', '| --- | ---: |');
  md.push(`| selected sets | ${selectedSets.length} |`);
  md.push(`| uncovered atomic effects | ${rows.length} |`);
  md.push(`| parser-recognized but unassigned atoms | ${parserRecognized} |`);
  ...
  md.push('## Complete List', '');
  md.push('| Set | Card | Effect | Atom | Parser | Action | Canonical | Text | Remark |', '| --- | --- | ---: | ---: | --- | --- | --- | --- | --- |');
  for (const row of rows) {
    const remark = row.trackingRemark || row.canonicalRemark || row.uncoveredReason;
    md.push(`| ${row.setCode} | ${row.cardNumber} ${row.name ? `(${escapeMd(row.name)})` : ''} | ${row.effectIndex} | ${row.atomIndex} | ${row.parserStatus} | ${row.actionType || 'UNMAPPED'} | ${row.canonicalAtoms || row.canonicalCoverage || ''} | ${escapeMd(truncate(row.rawText, 140))} | ${escapeMd(truncate(remark, 160))} |`);
  }
  md.push('');
...
function main(): void {
  ...
  const summary = {
    ...
    totalUncoveredAtomicEffects: rows.length,
    parserRecognizedUnassignedAtomicEffects: rows.filter((row) => row.parserStatus === 'recognized').length,
  ...
  console.log(
    `[effects:v2:uncovered] ${rows.length} uncovered atoms across ${setCodes.length} sets (${summary.parserUnrecognizedAtomicEffects} parser-unrecognized, ${summary.parserRecognizedUnassignedAtomicEffects} parser-recognized unassigned)`,
  );
}
