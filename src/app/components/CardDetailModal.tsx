...
export function CardDetailModal({ open, onClose, definition, imageUrl, setName, mobileImageOnly = false }: CardDetailModalProps) {
  ...
  return (
    <Modal
      open={open && definition !== null}
      onClose={onClose}
      rootClassName={mobileImageOnlyActive ? 'op-card-detail-modal-mobile-image' : undefined}
    ...
    >
      {definition && (
        <div className={['relative grid h-full min-h-0 grid-cols-[auto_minmax(0,1fr)] gap-4 overflow-hidden border-2 border-gold/35 bg-[#071126] p-4 text-slate-100 shadow-[0_18px_0_rgba(1,5,16,0.72),_0_34px_70px_rgba(0,0,0,0.46)]', mobileImageOnlyActive ? 'op-card-detail-content-mobile-image' : ''].filter(Boolean).join(' ')}>
