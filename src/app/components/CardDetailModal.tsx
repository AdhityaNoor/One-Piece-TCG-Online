...
export interface CardDetailModalProps {
  ...
  /**
   * Extra class(es) added to the portaled modal root (e.g. "op-theme-blue").
   * Modal portals into document.body, which escapes the DOM subtree of
   * whatever screen opened it — so a screen-scoped accent-color wrapper
   * (see index.css's .op-theme-blue) has to be re-applied here explicitly
   * rather than relying on CSS ancestry.
   */
...
function isSmallCardDetailViewport(): boolean {
  return typeof window !== 'undefined' && window.matchMedia('(max-width: 767px)').matches;
}

export function CardDetailModal({ open, onClose, definition, imageUrl, setName, mobileImageOnly = false, accentClassName }: CardDetailModalProps) {
  const [smallViewport, setSmallViewport] = useState(isSmallCardDetailViewport);
  ...
  const mobileImageOnlyActive = mobileImageOnly && smallViewport;

  if (mobileImageOnlyActive) {
    return (
      <Modal
        open={open && definition !== null}
        onClose={onClose}
        rootClassName={['op-card-detail-modal-mobile-image', accentClassName ?? ''].filter(Boolean).join(' ')}
  ...
  return (
    ...
    >
      {definition && (
        <div className={['relative grid h-full min-h-0 grid-cols-[auto_minmax(0,1fr)] gap-4 overflow-hidden border-2 border-[rgb(var(--op-gold-rgb)/0.35)] bg-[#071126] p-4 text-slate-100 shadow-[0_18px_0_rgba(1,5,16,0.72),_0_34px_70px_rgba(0,0,0,0.46)]', mobileImageOnlyActive ? 'op-card-detail-content-mobile-image' : ''].filter(Boolean).join(' ')}>
          ...
          <div className={['relative z-10 flex min-h-0 flex-col gap-4 overflow-hidden', mobileImageOnlyActive ? 'op-card-detail-info-mobile-image' : ''].filter(Boolean).join(' ')}>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {definition.colors.map((color) => (
                <span key={color} className="inline-flex items-center gap-1 border border-[rgb(var(--op-gold-rgb)/0.2)] bg-black/30 px-2 py-1 text-xs font-bold uppercase tracking-[0.08em] text-slate-100 shadow-[0_4px_0_rgba(1,5,16,0.34)]">
            ...
            {stats.length > 0 && (
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                {stats.map((stat) => (
                  <div key={stat.label} className="border border-[rgb(var(--op-gold-rgb)/0.2)] bg-black/30 px-3 py-2 text-center shadow-[0_5px_0_rgba(1,5,16,0.42)]">
                    <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[rgb(var(--op-gold-rgb)/0.7)]">{stat.label}</p>
            ...
            {definition.hasTrigger && (
              <div className="border border-[rgb(var(--op-gold-rgb)/0.45)] bg-black/55 p-3 shadow-[0_5px_0_rgba(68,39,0,0.5)]">
