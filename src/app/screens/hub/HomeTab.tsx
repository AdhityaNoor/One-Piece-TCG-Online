    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 p-4 sm:p-6 lg:p-8">
        <div className="inline-flex items-center gap-2">
          <span aria-hidden="true" className="h-1.5 w-1.5 flex-shrink-0 rounded-full bg-gold shadow-[0_0_10px_rgba(217,164,65,0.65)]" />
          <p className="text-[11px] font-black uppercase tracking-[0.28em] text-gold sm:text-xs">{slide.eyebrow}</p>
        </div>
        {slide.linkUrl ? (
          <a href={slide.linkUrl} target="_blank" rel="noreferrer" className="group inline-block">
            <h2 className="mt-1.5 font-display text-3xl font-black uppercase tracking-[0.05em] text-white drop-shadow-[0_4px_0_rgba(0,0,0,0.5)] group-hover:underline sm:text-4xl lg:text-5xl">
              {slide.title}
            </h2>
          </a>
        ) : (
          <h2 className="mt-1.5 font-display text-3xl font-black uppercase tracking-[0.05em] text-white drop-shadow-[0_4px_0_rgba(0,0,0,0.5)] sm:text-4xl lg:text-5xl">
            {slide.title}
          </h2>
        )}
        <p className="mt-2 max-w-md text-sm leading-6 text-slate-200/80 sm:text-base">{slide.body}</p>
      </div>
  ...
  const cards: ActionCardDef[] = [
    {
      ...
      tag: 'Online',
      tagClassName: 'border-cyan-300/50 bg-cyan-300/15 text-cyan-200',
    ...
    },
    {
      ...
      tag: 'Local',
      tagClassName: 'border-gold/50 bg-gold/15 text-gold',
    ...
    },
    {
      ...
      tag: 'Ranked',
      tagClassName: 'border-violet-300/50 bg-violet-300/15 text-violet-200',
    ...
    },
    {
      ...
      tag: 'Builder',
      tagClassName: 'border-emerald-300/50 bg-emerald-300/15 text-emerald-200',
    ...
    <nav className="flex h-full min-h-0 flex-col" aria-label="Play shortcuts">
      <div className="pb-3">
        <div className="inline-flex items-center gap-2">
          ...
          <p className="font-display text-sm font-black uppercase tracking-[0.22em] text-gold sm:text-base">Quick Actions</p>
        </div>
      </div>
        ...
        {cards.map((card) => (
          ...
          >
            ...
            <div className="relative z-10 flex h-full flex-col items-end justify-end gap-1 px-3 py-2 text-right sm:px-4 sm:py-3">
              ...
              <span className="font-display text-xl font-black uppercase tracking-[0.08em] text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.85)] transition-colors group-hover:text-red-600 sm:text-2xl lg:text-3xl">
                {card.title}
              </span>
              <span className="text-sm text-slate-100/80 drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)] sm:text-base">{card.subtitle}</span>
            </div>
          </button>
        ))}
      </div>
