...
export const EFFECT_PRIMITIVES: Record<AbilityFunction['fn'], CapabilitySpec> = {
  ...
  giveDon: {
    ...
    params: [
      ...
      { name: 'skipRestedDonGate', type: 'boolean', required: false, note: 'Use only when an earlier ordered function creates the rested DON!! this give step consumes.' },
    ],
  ...
  koAllCharacters: {
    id: 'koAllCharacters',
    summary: 'K.O. all matching Characters without a target choice. Defaults to both players; can restrict to controller/opponent and exclude the source.',
    params: [
      { name: 'player', type: "'any' | 'controller' | 'opponent'", required: false },
      { name: 'filter', type: '{ maxCost?; maxPower?; rested? }', required: false },
      { name: 'excludeSource', type: 'boolean', required: false },
    ],
    covers: ['K.O. all Characters with a cost of {C} or less', 'K.O. all Characters other than this Character', 'K.O. all of your opponent\'s Characters with a cost of {C} or less'],
    examples: [{ cardNumber: 'OP06-117', snippet: "{ fn: 'koAllCharacters', player: 'opponent', filter: { maxCost: 2 } }" }],
  },
  rest: {
    id: 'rest',
  ...
  restControllerCards: {
    id: 'restControllerCards',
    summary: 'Rest N of the controller\'s active cards as an effect-chain payment: Leader, Characters, Stage, or active DON!!.',
    params: [{ name: 'count', type: 'number', required: true }, { name: 'optional', type: 'boolean', required: false }],
    covers: ['you may rest {N} of your cards', 'rest {N} of your cards'],
    examples: [{ cardNumber: 'OP14-029', snippet: "{ fn: 'restControllerCards', count: 2, optional: true }" }],
  },
  returnOpponentDon: {
    id: 'returnOpponentDon',
    summary: 'Opponent chooses N DON!! from their field (or active cost-area only) and returns them to their DON!! deck.',
    params: [
      { name: 'count', type: 'number', required: true },
      { name: 'activeOnly', type: 'boolean', required: false, note: 'restrict to active unattached DON!! in the cost area' },
    ],
    covers: [
      'your opponent returns {N} DON!! card from their field to their DON!! deck',
      'your opponent may return {N} of their active DON!! cards to their DON!! deck',
    ],
    examples: [
      { cardNumber: 'OP14-065', snippet: "{ fn: 'returnOpponentDon', count: 1 }" },
      { cardNumber: 'OP15-059', snippet: "{ fn: 'returnOpponentDon', count: 1, activeOnly: true }" },
    ],
  },
  ...
  preventRefresh: {
    ...
    covers: ['up to {N} of your opponent\'s rested Characters will not become active in ... next Refresh Phase', 'up to {N} of your opponent\'s rested Characters or Stages will not become active in ... next Refresh Phase'],
    examples: [{ cardNumber: 'OP14-021', snippet: "{ fn: 'preventRefresh', target: { group: 'charactersOrStages', player: 'opponent', filter: { rested: true } }, optional: true }" }],
  },
  ...
  optionalRevealTypeFromHand: {
    ...
    summary: 'Optionally reveal N matching cards from hand (stays in hand); with upTo, reveal 1..N. Optional then branch runs after reveal.',
    params: [
      { name: 'filter', type: 'SearchFilter', required: false },
      { name: 'prompt', type: 'string', required: false },
      { name: 'count', type: 'number', required: false },
      { name: 'upTo', type: 'boolean', required: false, note: 'when true with count>1, reveal 1..count instead of exactly count' },
      { name: 'then', type: 'SequencedAbilityFunction[]', required: false },
    ],
    covers: [
      'you may reveal 1 {type} card from your hand',
      'you may reveal 2 {type}/power cards from your hand',
      'you may reveal up to 2 {type} Character cards from your hand',
      'reveal from hand as a cost, then resolve payoff functions',
    ],
    examples: [
      { cardNumber: 'OP13-024', snippet: "{ fn: 'optionalRevealTypeFromHand', filter: { anyOf: [{ typeIncludes: 'Music' }, { typeIncludes: 'FILM' }] } }" },
      { cardNumber: 'OP16-003', snippet: "{ fn: 'optionalRevealTypeFromHand', count: 2, filter: { category: 'character', exactPower: 8000 } }" },
      { cardNumber: 'OP10-058', snippet: "{ fn: 'optionalRevealTypeFromHand', count: 2, upTo: true, filter: { category: 'character', typeIncludes: 'Dressrosa', maxCost: 7, excludeSelfName: true }, then: [...] }" },
    ],
  },
  ...
  negateControllerEffects: {
    ...
    params: [
      ...
      { name: 'appliesToCategories', type: "('leader' | 'character' | 'event' | 'stage')[]", required: false },
      { name: 'exceptTypeIncludes', type: 'string', required: false },
    ],
    covers: ['Your [On Play] effects are negated', 'Your opponent\'s [On Play] effects are negated until the end of your opponent\'s next turn', 'Your Leader and Characters that do not have {type} have their effects negated'],
    examples: [
      { cardNumber: 'OP09-081', snippet: "{ fn: 'negateControllerEffects', player: 'controller', negatedTimings: ['onPlay'], duration: 'permanent' }" },
      { cardNumber: 'OP13-064', snippet: "{ fn: 'negateControllerEffects', player: 'controller', duration: 'permanent', appliesToCategories: ['leader', 'character'], exceptTypeIncludes: 'Roger Pirates' }" },
    ],
  },
  ...
  preventEffectDraw: {
    id: 'preventEffectDraw',
    summary: 'Controller cannot draw cards using their own effects for the duration. Draw Phase draws are unaffected.',
    params: [
      { name: 'duration', type: 'IrDuration', required: true },
      { name: 'player', type: "'controller' | 'opponent'", required: false },
    ],
    covers: ['you cannot draw cards using your own effects during this turn'],
    excludes: ['cannot draw during Draw Phase', 'opponent cannot draw'],
    examples: [{ cardNumber: 'OP12-099', snippet: "{ fn: 'preventEffectDraw', duration: 'duringThisTurn' }" }],
  },
  redirectFaceUpLifeToDeckBottom: {
    id: 'redirectFaceUpLifeToDeckBottom',
    summary: 'Face-up Life cards go to the bottom of the deck instead of hand (battle damage, effect Life→hand). Register at startOfGame on Leaders.',
    params: [{ name: 'duration', type: 'IrDuration', required: false, note: 'defaults to permanent' }],
    covers: ['Your face-up Life cards are placed at the bottom of your deck instead of being added to your hand'],
    excludes: ['face-down Life damaged normally (still go to hand)', 'Banish (still trashes)'],
    examples: [{ cardNumber: 'ST13-003', snippet: "{ fn: 'redirectFaceUpLifeToDeckBottom' }" }],
  },
  ...
  replaceEmptyDeckDefeatWithWin: {
    id: 'replaceEmptyDeckDefeatWithWin',
    summary: 'When your deck is reduced to 0, you win instead of losing (empty-deck defeat checkpoints + empty-deck draw attempts). Register at startOfGame on Leaders.',
    params: [{ name: 'duration', type: 'IrDuration', required: false, note: 'defaults to permanent' }],
    covers: [
      'When your deck is reduced to 0, you win the game instead of losing, according to the rules.',
    ],
    excludes: ['deferred empty-deck loss (use deferEmptyDeckDefeatToEndOfTurn)', 'unconditional win (use winGame)'],
    examples: [{ cardNumber: 'OP03-040', snippet: "{ fn: 'replaceEmptyDeckDefeatWithWin' }" }],
  },
  winGame: {
    id: 'winGame',
    summary: 'You win the game immediately (card-effect victory).',
    params: [],
    covers: ['you win the game'],
    excludes: ['win instead of losing from empty deck (use replaceEmptyDeckDefeatWithWin)'],
    examples: [{ cardNumber: 'OP09-118', snippet: "{ fn: 'winGame' }" }],
  },
  grantExtraTurn: {
    id: 'grantExtraTurn',
    summary: 'Take an extra turn after this one. Consumed at End Phase handoff (same player remains active).',
    params: [],
    covers: ['take an extra turn after this one'],
    excludes: ['skip opponent turn without prior grant', 'extra Main Phase only'],
    examples: [{ cardNumber: 'OP05-119', snippet: "{ fn: 'grantExtraTurn' }" }],
  },
  registerDonPhasePlacement: {
    id: 'registerDonPhasePlacement',
    summary: 'If you already have any DON!! on your field, 1 DON!! card placed during your DON!! Phase is given to your Leader. Register at startOfGame on Leaders.',
    params: [{ name: 'duration', type: 'IrDuration', required: false, note: 'defaults to permanent' }],
    covers: [
      'If you have any DON!! cards on your field, 1 DON!! card placed during your DON!! Phase is given to your Leader.',
    ],
    excludes: ['give DON!! outside DON!! Phase', 'choose which DON!! / which Character'],
    examples: [{ cardNumber: 'OP13-003', snippet: "{ fn: 'registerDonPhasePlacement' }" }],
  },
  dealDamage: {
    id: 'dealDamage',
  ...
  preventBlockers: {
    ...
    params: [
      { name: 'duration', type: 'IrDuration', required: true },
      { name: 'target', type: "'self' | 'controllerLeader' | 'chosenControllerLeaderOrCharacter'", required: false },
    ...
    covers: ['your opponent cannot activate [Blocker] during this battle', 'if your Leader attacks during this turn, your opponent cannot activate [Blocker]', 'if the selected card attacks during this turn, your opponent cannot activate [Blocker]', 'cannot activate the [Blocker] of any Character with a cost of {N} or less', 'cannot activate a [Blocker] Character that has {N} or less power'],
  ...
  addPower: {
    id: 'addPower',
    summary: 'Add/subtract power on chosen targets. Permanent + condition = a conditional continuous buff; a turn/battle duration = a one-shot pump. Optional `scale` for +N per count source.',
    params: [
      { name: 'target', type: 'TargetSpec', required: true },
      ...
      { name: 'condition', type: 'IrCondition', required: false },
      { name: 'optional', type: 'boolean', required: false },
      { name: 'scale', type: 'PowerScale', required: false, note: "e.g. { per: 'controllerCharacters', step: 1, amountPer: 1000 }" },
    ],
    covers: [
      'up to {N} of your Leader or Character cards gains +{P} power during this turn',
      'give up to 1 of your opponent\'s Characters −{P} power',
      'Your Leader gains +{P} power for each of your Characters during this turn',
    ],
    examples: [
      { cardNumber: 'OP12-057', snippet: "{ fn: 'addPower', target: { group: 'leaderOrCharacters', player: 'controller' }, amount: 4000, duration: 'duringThisBattle', optional: true }" },
      { cardNumber: 'P-024', snippet: "{ fn: 'addPower', target: { group: 'leader', player: 'controller' }, amount: 0, duration: 'duringThisTurn', scale: { per: 'controllerCharacters', step: 1, amountPer: 1000 } }" },
    ],
  },
  addPowerSelf: {
    id: 'addPowerSelf',
  ...
  setBasePowerFromSource: {
    id: 'setBasePowerFromSource',
    summary: 'Set base power to match another card\'s current power at resolution time.',
    params: [
      { name: 'target', type: 'TargetSpec', required: true },
      { name: 'source', type: 'TargetSpec', required: true },
      { name: 'duration', type: 'IrDuration', required: true },
      { name: 'optional', type: 'boolean', required: false },
      { name: 'maxTargets', type: 'number', required: false },
      { name: 'condition', type: 'IrCondition', required: false },
    ...
    covers: [
      'this Character\'s base power becomes the same as your opponent\'s Leader\'s power',
      'this Character\'s base power becomes the same as the selected Character\'s power',
    ],
    examples: [
      { cardNumber: 'OP16-036', snippet: "{ fn: 'setBasePowerFromSource', target: { ref: 'self' }, source: { group: 'leader', player: 'opponent' }, duration: 'duringThisTurn' }" },
      { cardNumber: 'OP16-104', snippet: "{ fn: 'setBasePowerFromSource', target: { ref: 'self' }, source: { group: 'characters', player: 'opponent' }, duration: 'duringThisTurn', optional: true }" },
    ],
  },
  drawByEventCount: {
    id: 'drawByEventCount',
  ...
  returnHandShuffleDraw: {
    ...
    summary: 'Return all hand cards to deck (shuffle or bottom), then draw equal count (or fixed drawAmount). Optional → chooseOption do/don\'t.',
    params: [
      { name: 'player', type: "'controller' | 'opponent'", required: false },
      { name: 'drawAmount', type: 'number', required: false },
      { name: 'destination', type: "'shuffle' | 'bottom'", required: false, note: "default 'shuffle'; 'bottom' places hand under deck in current order (no interactive reorder)" },
      { name: 'optional', type: 'boolean', required: false, note: 'Wraps in chooseOption skip/return' },
    ],
    covers: [
      ...
      'You may place all cards in your hand at the bottom of your deck in any order. If you do, draw cards equal to the number you placed at the bottom of your deck.',
    ],
    examples: [
      ...
      { cardNumber: 'P-046', snippet: "{ fn: 'returnHandShuffleDraw', destination: 'bottom', optional: true }" },
    ],
  },
  ...
  addAttribute: {
    id: 'addAttribute',
    summary: 'Grant a printed attribute (<Slash>, <Strike>, etc.) to targets for a duration. Pair with addKeyword when a card also gains a keyword.',
    params: [
      { name: 'target', type: 'TargetSpec', required: true },
      { name: 'attribute', type: 'Attribute', required: true },
      { name: 'duration', type: 'IrDuration', required: true },
      { name: 'condition', type: 'IrCondition', required: false },
      { name: 'optional', type: 'boolean', required: false },
    ],
    covers: ['gains the <Slash> attribute during this turn', 'gains the <Strike>/<Ranged>/<Special>/<Wisdom> attribute'],
    excludes: ['changing printed attribute permanently on the card definition'],
    examples: [{ cardNumber: 'OP15-093', snippet: "{ fn: 'addAttribute', target: { group: 'characters', player: 'controller', filter: { name: 'Monkey.D.Luffy' } }, attribute: 'slash', duration: 'duringThisTurn', optional: true }" }],
  },
  ...
  trashTopDeck: {
    id: 'trashTopDeck',
    summary: 'Trash N cards from the top of the controller\'s deck (self-mill). With countVar, N is the length of a prior selection binding.',
    params: [{ name: 'count', type: 'number', required: false }, { name: 'countVar', type: 'string', required: false }],
    covers: ['trash {N} cards from the top of your deck', 'trash the same number of cards from the top of your deck as you did from your hand'],
    examples: [
      { cardNumber: 'OP04-079', snippet: "{ fn: 'trashTopDeck', count: 2 }" },
      { cardNumber: 'OP09-059', snippet: "{ fn: 'optionalTrashFromHand', count: 2 }, { fn: 'trashTopDeck', countVar: 't', ifPrevious: 'previousMovedAny' }" },
    ],
  },
  trashSelf: {
    id: 'trashSelf',
  ...
  returnSelfToHand: {
    id: 'returnSelfToHand',
    summary: 'Move the source card instance to its owner\'s hand immediately.',
    params: [],
  ...
  moveCards: {
    id: 'moveCards',
    summary: 'Generic zone move: from a source (characters/hand/trash/deck/life) to a destination (hand/life/deck-top/bottom/topOrBottom/trash). Covers return-to-hand, bottom-deck, life-to-hand, etc.',
    params: [
    ...
    covers: ['return up to 1 Character to the owner\'s hand', 'place ... at the bottom of the owner\'s deck', 'place 1 card from your hand at the top or bottom of your deck', 'add 1 card from the top of your Life to your hand', 'your opponent places 1 card from their hand at the bottom of their deck', 'move/K.O. a Character with a [Trigger]'],
    ...
    examples: [
      { cardNumber: 'OP01-047', snippet: "{ fn: 'moveCards', from: { zone: 'characters', player: 'any' }, to: { zone: 'hand', player: 'owner' }, optional: true }" },
      { cardNumber: 'OP08-056', snippet: "{ fn: 'moveCards', from: { zone: 'hand', player: 'controller' }, to: { zone: 'deck', player: 'owner', position: 'topOrBottom' }, maxTargets: 1 }" },
      { cardNumber: 'OP09-115', snippet: "{ fn: 'ko', target: { group: 'characters', player: 'opponent', filter: { maxCost: 3, hasTrigger: true } }, optional: true }" },
    ],
  },
  moveAllCards: {
    id: 'moveAllCards',
  ...
  moveAllCharactersToBottomDeck: {
    ...
    params: [
      { name: 'filter', type: '{ maxCost?; maxPower?; maxBaseCost?; maxBasePower?; excludeSelf?; player? }', required: false },
    ],
    covers: [
      'Place all Characters with a cost of {N} or less at the bottom of the owner\'s deck',
      'place all of your Characters except this Character at the bottom of your deck',
    ],
    examples: [
      { cardNumber: 'OP05-058', snippet: "{ fn: 'moveAllCharactersToBottomDeck', filter: { maxCost: 3 } }" },
      { cardNumber: 'OP05-119', snippet: "{ fn: 'moveAllCharactersToBottomDeck', filter: { player: 'controller', excludeSelf: true } }" },
    ],
  },
  peekLifeAndPlace: {
    id: 'peekLifeAndPlace',
  ...
  playFromHand: {
    id: 'playFromHand',
    summary: 'Play up to N cards matching a filter from hand, or choose from a prior binding via fromVar (e.g. revealed set).',
    params: [
      { name: 'filter', type: 'SearchFilter', required: false, note: 'required unless fromVar is set' },
      { name: 'fromVar', type: 'string', required: false, note: 'play a subset of a prior chooseTargets/captureCount binding' },
      { name: 'maxTargets', type: 'number', required: false },
      { name: 'minTargets', type: 'number', required: false },
      { name: 'optional', type: 'boolean', required: false, note: 'with fromVar, optional:false forces minTargets 1' },
    ...
    covers: [
      ...
      'play ... with a cost equal to or less than the number of DON!! cards on your/opponent\'s field',
      'play ... from your hand rested',
      'play ... a different color than the returned Character',
      'opponent plays up to 1 Character card with a cost of {C} or less from their hand',
      'play up to N Characters with different card names from your hand',
      'play 1 of the revealed cards',
      'play the other card rested if it has a cost of {C} or less',
    ],
    examples: [
      { cardNumber: 'EB02-028', snippet: "{ fn: 'playFromHand', filter: { category: 'character', exactCost: 2 }, rested: true }" },
      { cardNumber: 'OP10-058', snippet: "{ fn: 'playFromHand', fromVar: 'revealed', maxTargets: 1, optional: false }" },
    ],
  },
  ...
  playPairOneRested: {
    id: 'playPairOneRested',
    summary: 'Choose up to 1 trash card per pick filter (distinct), then if 2 were chosen pick which enters rested and play the other active; a lone pick plays active.',
    params: [
      { name: 'zone', type: "'trash'", required: true },
      { name: 'picks', type: '[{ filter: SearchFilter; prompt?: string }, { filter: SearchFilter; prompt?: string }]', required: true },
    ],
    covers: [
      'choose up to 1 Character card with a cost of {A} or less and up to 1 Character card with a cost of {B} or less from your trash. Play 1 card and play the other card rested',
    ],
    examples: [{
      cardNumber: 'OP06-086',
      snippet: "{ fn: 'playPairOneRested', zone: 'trash', picks: [{ filter: { category: 'character', maxCost: 4 } }, { filter: { category: 'character', maxCost: 2 } }] }",
    }],
  },
  playSelfFromTrash: {
    id: 'playSelfFromTrash',
    summary: 'Play the source Character from its owner trash.',
    params: [],
    covers: ['Play this Character card from your trash'],
    excludes: ['[Trigger] Play this card while it is still in Life/hand; use triggerPlaySelf instead'],
    examples: [{ cardNumber: 'OP14-120', snippet: "{ fn: 'playSelfFromTrash', ifPrevious: 'previousMovedAny' }" }],
  },
  triggerPlaySelf: {
    id: 'triggerPlaySelf',
  ...
  searchTopDeck: {
    ...
    covers: ['look at {N} cards from the top of your deck; reveal up to 1 {type} card and add it to your hand', 'reveal up to 1 <attribute> card or {color} Event and add it to your hand', 'reveal 1 card from the top of your deck, play up to 1 Character with a cost of 2, and place the rest at the top or bottom of your deck', 'place the rest at the top or bottom of the deck in any order'],
    examples: [
      ...
      { cardNumber: 'OP06-057', snippet: "{ fn: 'searchTopDeck', look: 1, pick: 1, reveal: true, destination: 'play', filter: { category: 'character', exactCost: 2 }, remainder: 'deckTopOrBottom' }" },
  ...
  captureCount: {
    id: 'captureCount',
    summary: 'Snapshot the previous selection (var `from`, default t) into stable var `into`, so a later chooseTargets reusing t does not clobber it (countVar scaling or excludeIdsFromVar).',
    params: [
      { name: 'from', type: 'string', required: false, note: "source var, default 't'" },
      { name: 'into', type: 'string', required: true, note: 'stable var read by a later addPower countVar or TargetFilter.excludeIdsFromVar' },
    ],
    covers: [
      '... gains +N power for every returned/K.O.\'d Character (when the buff recipient is separately chosen)',
      'give 1 Character −A and the other −B (exclude prior pick via excludeIdsFromVar)',
    ],
    examples: [
      { cardNumber: 'P-059', snippet: "{ fn: 'captureCount', into: 'returned' }, { fn: 'addPower', target: { group: 'leaderOrCharacters', player: 'controller' }, amount: 0, countVar: 'returned', amountPer: 2000, duration: 'duringThisBattle', optional: true, maxTargets: 1 }" },
  ...
  setActiveControllerCharacters: {
    ...
    params: [{ name: 'filter', type: '{ maxCost?; exactCost?; rested?; typeIncludes?; anyOfTypes? }', required: false }],
    covers: ['set all of your Characters as active'],
    examples: [{ cardNumber: 'OP16-038', snippet: "{ fn: 'setActiveControllerCharacters' }" }],
  },
  ...
  setActiveControllerStage: {
    id: 'setActiveControllerStage',
    summary: 'Set up to N of the controller\'s Stages as active (optional color/rested filter).',
    params: [
      { name: 'filter', type: '{ color?; rested? }', required: false },
      { name: 'maxTargets', type: 'number', required: false, note: 'defaults to 1' },
      { name: 'optional', type: 'boolean', required: false },
    ],
    covers: ['set up to 1 of your purple Stages as active', 'set up to {N} of your Stages as active'],
    examples: [{ cardNumber: 'P-077', snippet: "{ fn: 'setActiveControllerStage', filter: { color: 'purple' }, maxTargets: 1 }" }],
  },
  ...
  trashSelfAtEndOfTurn: {
    id: 'trashSelfAtEndOfTurn',
    summary: 'Schedule the source Character to be trashed at the end of this turn.',
    params: [],
    covers: ['trash this Character at the end of this turn'],
    examples: [{ cardNumber: 'OP03-005', snippet: "{ fn: 'trashSelfAtEndOfTurn' }" }],
  },
  moveSelfToBottomDeckAtEndOfBattle: {
    id: 'moveSelfToBottomDeckAtEndOfBattle',
    summary: 'Schedule the source Character to be placed at the bottom of its owner\'s deck at the end of this battle.',
    params: [],
  ...
  movePreviousMovedToBottomDeckAtEndOfTurn: {
    ...
    covers: ['place the Character played by this effect at the bottom of the owner\'s deck at the end of this turn'],
    examples: [{ cardNumber: 'OP11-092', snippet: "{ fn: 'movePreviousMovedToBottomDeckAtEndOfTurn', ifPrevious: 'previousMovedAny' }" }],
  },
  trashControllerCharacterAtEndOfTurn: {
    id: 'trashControllerCharacterAtEndOfTurn',
    summary: 'Schedule one of the controller\'s Characters (optionally type-filtered) to be trashed at end of turn.',
    params: [{ name: 'filter', type: '{ typeIncludes?: string }', required: false }],
    covers: ['trash 1 of your {Type} Characters at the end of this turn'],
    examples: [{ cardNumber: 'OP06-006', snippet: "{ fn: 'trashControllerCharacterAtEndOfTurn', filter: { typeIncludes: 'FILM' } }" }],
  },
  returnDonToMatchOpponentAtEndOfTurn: {
    id: 'returnDonToMatchOpponentAtEndOfTurn',
    summary: 'Schedule returning DON!! from your field to your DON!! deck until your field count matches your opponent\'s.',
    params: [],
    covers: ['at the end of this turn, return DON!! cards from your field to your DON!! deck until you have the same number of DON!! cards on your field as your opponent'],
    examples: [{ cardNumber: 'OP08-074', snippet: "{ fn: 'returnDonToMatchOpponentAtEndOfTurn' }" }],
  },
  moveDeckTopToLifeAtEndOfTurn: {
    id: 'moveDeckTopToLifeAtEndOfTurn',
    summary: 'Schedule adding the top card of your deck to the top of your Life at end of turn (optional leader gate at resolution).',
    params: [
      { name: 'gates', type: 'AbilityGate[]', required: false },
      { name: 'ifPrevious', type: 'SequenceCondition', required: false },
    ],
    covers: ['add 1 card from the top of your deck to the top of your Life cards at the end of this turn'],
    examples: [{ cardNumber: 'OP08-101', snippet: "{ fn: 'moveDeckTopToLifeAtEndOfTurn', gates: [{ kind: 'leaderType', type: 'Big Mom Pirates' }], ifPrevious: 'previousMovedAny' }" }],
  },
  trashHandDownTo: {
    id: 'trashHandDownTo',
    summary: 'Trash cards from a player\'s hand until they have the given hand size (auto-discards from hand end; no per-card choice).',
    params: [{ name: 'handSize', type: 'number', required: true }, { name: 'player', type: "'controller' | 'opponent'", required: false }],
    covers: ['trash cards from your hand until you have {N} cards in your hand', 'you and your opponent trash cards from your hands until you each have {N} cards'],
    examples: [
      { cardNumber: 'OP14-054', snippet: "{ fn: 'trashHandDownTo', handSize: 5 }" },
      { cardNumber: 'OP05-058', snippet: "{ fn: 'trashHandDownTo', handSize: 5 }, { fn: 'trashHandDownTo', handSize: 5, player: 'opponent' }" },
    ],
  },
  trashFaceUpLife: {
    id: 'trashFaceUpLife',
    summary: 'Trash all face-up Life cards.',
    params: [],
  ...
  addCounterAuraControllerCharactersInHand: {
    id: 'addCounterAuraControllerCharactersInHand',
    summary: 'Dynamic Counter aura over Character cards in the controller\'s hand ("+1000 Counter" / "Counter becomes +2000"). Consumed by ACTIVATE_COUNTER_CHARACTER via computeEffectiveCounter.',
    params: [
      { name: 'amount', type: 'number', required: false, note: 'additive Counter delta; mutually exclusive with setValue in practice' },
      { name: 'setValue', type: 'number', required: false, note: 'overwrite Counter ("becomes +N")' },
      { name: 'duration', type: 'IrDuration', required: true },
      { name: 'anyOfTypes', type: 'string[]', required: false },
      { name: 'exactBasePower', type: 'number', required: false, note: 'printed base power must equal this (hand Characters)' },
      { name: 'withoutPrintedCounter', type: 'true', required: false, note: 'only Characters with no printed Counter' },
      { name: 'sourceCondition', type: 'SourceStateCondition', required: false },
      { name: 'gate', type: 'AbilityGate[]', required: false },
    ],
    covers: [
      'all of your {type} type Character cards without a Counter have a +{N} Counter',
      'the counter of all of your Character cards with {P} power in your hand becomes +{N}',
    ],
    examples: [
      { cardNumber: 'EB01-001', snippet: "{ fn: 'addCounterAuraControllerCharactersInHand', amount: 1000, duration: 'permanent', anyOfTypes: ['Land of Wano'], withoutPrintedCounter: true }" },
      { cardNumber: 'OP16-118', snippet: "{ fn: 'addCounterAuraControllerCharactersInHand', setValue: 2000, duration: 'permanent', exactBasePower: 8000 }" },
    ],
  },
  ...
  addCostAuraControllerCharacters: {
    ...
    summary: 'Continuous cost aura over ALL controller Characters (chars only), optionally type/color/base-cost-filtered + source-state gated.',
    params: [
      ...
      { name: 'minBaseCost', type: 'number', required: false, note: 'restrict to Characters with printed cost >= N' },
      { name: 'maxBaseCost', type: 'number', required: false, note: 'restrict to Characters with printed cost <= N' },
      { name: 'sourceCondition', type: 'SourceStateCondition', required: false },
      { name: 'gate', type: 'AbilityGate[]', required: false, note: 'board gate re-evaluated each read against the source\'s controller (e.g. "if you have 3+ {type} Characters")' },
    ],
    covers: ['[Opponent\'s Turn] all of your {type} Characters gain +{C} cost', 'all of your Characters gain +{C} cost', 'all of your {color} {type} Characters gain +{C} cost', 'all of your {type} Characters with a cost of {N} or more gain +{C} cost', 'if <board state>, all of your {type} Characters gain +{C} cost'],
    examples: [
      { cardNumber: 'EB04-046', snippet: "{ fn: 'addCostAuraControllerCharacters', amount: 2, duration: 'permanent', anyOfTypes: ['Navy'], sourceCondition: { turn: 'opponent' } }" },
      { cardNumber: 'OP10-042', snippet: "{ fn: 'addCostAuraControllerCharacters', amount: 1, duration: 'permanent', anyOfTypes: ['Dressrosa'], minBaseCost: 2 }" },
    ],
  },
  ...
  addCounterAuraSameCardInHand: { id: 'addCounterAuraSameCardInHand', summary: "Counter twin of addCostAuraSameCardInHand: 'this card in your hand has a +N Counter'. Read from the card's own program while in hand (handSelfCounterDelta), so it applies to the first copy in hand, not only to duplicates once one is on the field.", params: [{ name: 'amount', type: 'number', required: true }, { name: 'duration', type: 'IrDuration', required: true }, { name: 'gate', type: 'AbilityGate[]', required: false }], covers: ['this card in your hand has a +{N} Counter'], examples: [{ cardNumber: 'OP17-118', snippet: "{ fn: 'addCounterAuraSameCardInHand', amount: 2000, duration: 'permanent', gate: [{ kind: 'selfAllCharactersWithoutCounter' }] }" }] },
  ...
  koImmunitySelf: {
    ...
    params: [
      ...
      { name: 'attackerWithoutAttribute', type: 'string', required: false, note: 'battle K.O. only — attacker must not have this attribute' },
    ...
    covers: [
      ...
      'this Character cannot be K.O.\'d in battle by <Attribute> attribute Characters',
      'this Character cannot be K.O.\'d in battle by Characters without the <Attribute> attribute',
      ...
      'cannot be K.O.\'d by effects of opponent Characters with {N} base power or less',
    ...
    examples: [
      ...
      { cardNumber: 'P-025', snippet: "{ fn: 'koImmunitySelf', scope: 'battle', duration: 'permanent', condition: { donAttachedAtLeast: 1 }, attackerCategory: 'character', attackerWithoutAttribute: 'special' }" },
  ...
  registerKoReplacementSelf: {
    ...
    params: [
      ...
      { name: 'restCards', type: '{ count?: number }', required: false, note: 'Rest Leader/Characters/Stage/active DON!!' },
  ...
  registerKoReplacementAura: {
    ...
    covers: [
      ...
      'If your Character would be removed from the field by your opponent\'s effect, you may rest 2 of your cards instead',
    ...
    examples: [
      ...
      { cardNumber: 'P-111', snippet: "{ fn: 'registerKoReplacementAura', scope: 'effect', oncePerTurn: true, anyOfTypes: ['Straw Hat Crew'], replacementTriggers: ['ko', 'returnToHand', 'bottomDeck'], effectSourceController: 'opponent', restDon: { count: 1 }, duration: 'permanent' }" },
...
export const GATES: Record<AbilityGate['kind'], CapabilitySpec> = {
  ...
  leaderColor: { id: 'leaderColor', summary: 'If your Leader has the specified color.', params: [{ name: 'color', type: 'Color', required: true }], covers: ['If your Leader\'s colors include {color}'], examples: [{ cardNumber: 'OP11-080', snippet: "{ kind: 'leaderColor', color: 'blue' }" }] },
  ...
  selfDonAtLeastLessThanOpponent: { id: 'selfDonAtLeastLessThanOpponent', summary: 'Your DON!! on field is at least N less than opponent\'s.', params: [{ name: 'count', type: 'number', required: true }], covers: ['If the number of DON!! cards on your field is at least {N} less than the number on your opponent\'s field'], examples: [{ cardNumber: 'OP07-064', snippet: "{ kind: 'selfDonAtLeastLessThanOpponent', count: 2 }" }] },
  selfControlsNamed: { id: 'selfControlsNamed', summary: 'You control/have a named card among your Leader, Characters, or Stage.', params: [{ name: 'name', type: 'string', required: true }], covers: ['If you have [{name}]', 'If you have a [{name}] Character'], excludes: ['"other than THIS Character" self-instance exclusion'], examples: [{ cardNumber: 'OP07-030', snippet: "{ kind: 'selfControlsNamed', name: 'Camie' }" }] },
  selfDoesNotControlNamed: { id: 'selfDoesNotControlNamed', summary: 'You do not control/have a named card among your Leader, Characters, or Stage.', params: [{ name: 'name', type: 'string', required: true }], covers: ['If you don\'t have [{name}]', 'you have no other [{name}] Characters'], examples: [{ cardNumber: 'EB01-012', snippet: "{ kind: 'selfDoesNotControlNamed', name: 'Cavendish' }" }] },
  ...
  selfTypedCharacterCount: { id: 'selfTypedCharacterCount', summary: 'You have N or more/less {type} Characters (counts self if same type).', params: [{ name: 'typeIncludes', type: 'string', required: true }, { name: 'atLeast', type: 'number', required: false }, { name: 'atMost', type: 'number', required: false }, { name: 'rested', type: 'boolean', required: false }, { name: 'color', type: 'Color', required: false }, { name: 'minCost', type: 'number', required: false, note: 'current cost floor on each counted Character' }], covers: ['if you have {N} or more {type} Characters', 'if you have a {type} type Character with a cost of {M} or more'], excludes: ['combined-OR of two types', '"other than [X]" self exclusion (approximate with atLeast+1)'], examples: [{ cardNumber: 'OP13-009', snippet: "{ kind: 'selfTypedCharacterCount', typeIncludes: 'Mountain Bandits', atLeast: 2 }" }, { cardNumber: 'EB01-001', snippet: "{ kind: 'selfTypedCharacterCount', typeIncludes: 'Land of Wano', minCost: 5, atLeast: 1 }" }] },
  selfTypedCharacterDistinctNameCount: { id: 'selfTypedCharacterDistinctNameCount', summary: 'You have N or more {type} Characters with different card names.', params: [{ name: 'typeIncludes', type: 'string', required: true }, { name: 'atLeast', type: 'number', required: true }], covers: ['if you have {N} {type} Characters with different card names'], examples: [{ cardNumber: 'OP16-038', snippet: "{ kind: 'selfTypedCharacterDistinctNameCount', typeIncludes: 'Impel Down', atLeast: 5 }" }] },
  ...
  selfAllCharactersWithoutCounter: { id: 'selfAllCharactersWithoutCounter', summary: 'You control no Character with a printed Counter. Vacuously TRUE when you control no Characters at all.', params: [], covers: ['if you only have Characters without a Counter'], examples: [{ cardNumber: 'OP17-118', snippet: "{ kind: 'selfAllCharactersWithoutCounter' }" }] },
  ...
  opponentCharacterCurrentPowerCount: { id: 'opponentCharacterCurrentPowerCount', summary: 'Opponent has N Characters with M current power or more.', params: [{ name: 'power', type: 'number', required: true }, { name: 'atLeast', type: 'number', required: false }, { name: 'atMost', type: 'number', required: false }], covers: ['if your opponent has a Character with {N} power or more', 'your opponent has {N} or more Characters with {M} power or more'], examples: [{ cardNumber: 'OP09-019', snippet: "{ kind: 'opponentCharacterCurrentPowerCount', power: 5000, atLeast: 1 }" }] },
  ...
  boundVarsTotalCount: {
    id: 'boundVarsTotalCount',
    summary: 'Sequence-local: total length of named chooseTargets bindings (requires GateEvalContext.bindings).',
    params: [
      { name: 'varNames', type: 'string[]', required: true },
      { name: 'atLeast', type: 'number', required: false },
      { name: 'atMost', type: 'number', required: false },
    ],
  ...
  playedCharacterHasTrigger: { id: 'playedCharacterHasTrigger', summary: 'The just-played Character has printed [Trigger] metadata.', params: [], covers: ['when you play a Character with a [Trigger]'], examples: [{ cardNumber: 'OP13-100', snippet: "{ kind: 'playedCharacterHasTrigger' }" }] },
  ...
  koedCharacterTypeIncludes: { id: 'koedCharacterTypeIncludes', summary: "The K.O.'d Character carries a type (onCharacterKoed only).", params: [{ name: 'typeIncludes', type: 'string', required: true }], covers: ["When your {Type} type Character is K.O.'d"], examples: [{ cardNumber: 'OP10-042', snippet: "{ kind: 'koedCharacterTypeIncludes', typeIncludes: 'Dressrosa' }" }] },
  koedCharacterAnyOfTypes: { id: 'koedCharacterAnyOfTypes', summary: "The K.O.'d Character carries any of the listed types (onCharacterKoed only).", params: [{ name: 'anyOfTypes', type: 'string[]', required: true }], covers: ["When your {A} or {B} type Character is K.O.'d"], examples: [{ cardNumber: 'OP14-041', snippet: "{ kind: 'koedCharacterAnyOfTypes', anyOfTypes: ['Amazon Lily', 'Kuja Pirates'] }" }] },
  koedCharacterMinBasePower: { id: 'koedCharacterMinBasePower', summary: "The K.O.'d Character has printed base power ≥ N (onCharacterKoed only).", params: [{ name: 'power', type: 'number', required: true }], covers: ["Characters with {N} base power or more is K.O.'d"], examples: [{ cardNumber: 'OP14-041', snippet: "{ kind: 'koedCharacterMinBasePower', power: 5000 }" }] },
  koByOpponentEffect: {
  ...
  noneOf: { id: 'noneOf', summary: 'NOR: satisfied if no nested sub-gate holds.', params: [{ name: 'gates', type: 'AbilityGate[]', required: true }], covers: ['if you do not have a Character with a cost of {N} or more'], examples: [{ cardNumber: 'OP04-040', snippet: "{ kind: 'noneOf', gates: [{ kind: 'selfHasCharacterCostAtLeast', atLeast: 8 }] }" }] },
};
