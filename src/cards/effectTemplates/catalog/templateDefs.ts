...
export type AbilityFunction =
  ...
  | { fn: 'preventAttackAll'; duration: IrDuration; forbiddenTarget?: 'leader'; player?: 'controller' | 'opponent' | 'both'; charactersOnly?: true; condition?: IrCondition; attackUnlessTrashFromHand?: number }
  | { fn: 'forceCharactersPlayedRested'; duration: IrDuration }
  ...
  | { fn: 'preventFieldRemovalAuraOpponentCharacters'; duration: IrDuration; anyOfTypes?: string[]; anyOfNames?: string[]; anyOfColors?: Color[]; targetCondition?: IrCondition; gate?: AbilityGate[]; effectSourceController?: 'opponent' | 'controller' }
  ...
  // Snapshot how many cards the controller has ON THE FIELD (Leader + Characters + Stages)
  // matching `typeIncludes`, into `into`. For "For every {Type} card on your field, ..."
  // clauses, whose count is fixed at RESOLUTION — unlike a continuous `scale`, which is
  // re-read on every power lookup and would drift as the field changes later in the turn.
  // Pair with `addPower { amount: 0, amountPer: N, countVar: into }` — countVar must be set
  // explicitly, since amountPer defaults its var to 't', which a targeted addPower has already
  // rebound to the CHOSEN TARGETS (so the scale would read 1, not the field count).
  | { fn: 'captureFieldTypeCount'; typeIncludes?: string; into: string }
  ...
  | { fn: 'setBasePowerAuraControllerTypes'; value: number; duration: IrDuration; anyOfTypes?: string[]; anyOfNames?: string[]; hasTrigger?: true; exactBasePower?: number; charactersOnly?: true; sourceCondition?: SourceStateCondition; gate?: AbilityGate[] }
  ...
  | { fn: 'revealTopLifeAddPowerPerCost'; amountPer: number; duration: IrDuration }
